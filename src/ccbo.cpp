...
//    qDebug() << "systemdirectory: " << systemdirectory;
...
//    }
    ...
    QString SNAP = env1.value("SNAP");

    QString path = SNAP + "/KylinLunar.db";
//    qDebug() << "systempathxxxxxxxxx1: " << systempath;

    QString userPath = env1.value("SNAP_USER_DATA");
    userPath += "/KylinLunar.db";
//    qDebug() << "userPath: " << userPath;

    if(!QFile::exists(userPath))
    {
        if (QFile::copy(path, userPath) == false) {
            qDebug() << "making the copy failed";
            return false;
        }
    }
